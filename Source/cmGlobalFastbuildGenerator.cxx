...
#define FASTBUILD_DOLLAR_TAG "FASTBUILD_DOLLAR_TAG"
                           ...
                           const std::string& configName)
  {
    if (gt->GetType() == cmStateEnums::EXECUTABLE) {
      gt->GetExecutableNames(targetNamesOut.targetNameOut,
    ...
    if (gt->HaveWellDefinedOutputFiles()) {
      targetNamesOut.targetOutputDir = gt->GetDirectory(configName) + "/";

      targetNamesOut.targetOutput = gt->GetFullPath(configName);
      targetNamesOut.targetOutputReal = gt->GetFullPath(configName,
                                                        /*implib=*/false,
                                                        /*realpath=*/true);
      targetNamesOut.targetOutputImplib = gt->GetFullPath(configName,
                                                          /*implib=*/true);
    } else {
      targetNamesOut.targetOutputDir =
        gt->Target->GetMakefile()->GetCurrentBinaryDirectory();
      if (targetNamesOut.targetOutputDir.empty() ||
          targetNamesOut.targetOutputDir == ".") {
        targetNamesOut.targetOutputDir = gt->GetName();
      } else {
        targetNamesOut.targetOutputDir += "/";
        targetNamesOut.targetOutputDir += gt->GetName();
      }
                                    ...
                                    const std::string& language)
  {
    ...
    lg->AddCompileDefinitions(defines, gt, configName, language);
                                   ...
                                   const std::string& configName)
  {
    ...
    for (std::vector<std::string>::const_iterator libDir = libDirs.begin();
      ...
      std::string libpath =
        lg->ConvertToOutputForExisting(*libDir, cmLocalGenerator::SHELL);
                                      ...
                                      const std::string& configName)
  {
    ...
    vars.CMTargetType=cmState::GetTargetTypeName(gt->GetType());
    ...
    std::string launcher;
    const char* val = lg->GetRuleLauncher(
      gt, "RULE_LAUNCH_LINK");
    if (val && *val) {
      launcher = val;
      launcher += " ";
    ...
    for (std::vector<std::string>::iterator i = linkCmds.begin();
         i != linkCmds.end(); ++i) {
      *i = launcher + *i;
      rulePlaceholderExpander->ExpandRuleVariables(lg,*i,vars);
    }
                                       ...
                                       const std::string& language)
  {
    ...
    compileObjectVars.CMTargetName=gt->GetName().c_str();
    compileObjectVars.CMTargetType=cmState::GetTargetTypeName(gt->GetType());
    ...
    for (std::vector<std::string>::iterator i = compileCmds.begin();
      ...
      rulePlaceholderExpander->ExpandRuleVariables(lg, compileCmdStr,
                                                   compileObjectVars);
    }
                                       ...
                                       const std::string& lang)
  {
    ...
    vars.CMTargetName=cmState::GetTargetTypeName(target->GetType());
    ...
    for (std::vector<std::string>::iterator i = compileCmds.begin();
         i != compileCmds.end(); ++i) {
      std::string& compileCmdStr = *i;
      compileCmdStr = launcher + compileCmdStr;

      rulePlaceholderExpander->ExpandRuleVariables(lg,compileCmdStr, vars);
    }
                              ...
                              const cmGeneratorTarget* gt)
  {
    ...
    // nor utilities
    bool hasObjectGroups = gt->GetType() != cmStateEnums::UTILITY &&
                                  ...
                                  const std::string& configName)
  {
    ...
    lg->AddCompileOptions(compileFlags, gt, language, configName);
  ...
  static void DetectTargetCompileDependencies(
    cmGlobalFastbuildGenerator* gg, const cmGeneratorTarget* gt,
    std::vector<std::string>& dependencies)
  {
    if (gt->GetType() == cmStateEnums::GLOBAL_TARGET) {
      // Global targets only depend on other utilities, which may not appear in
      // the TargetDepends set (e.g. "all").
      std::set<std::string> const& utils = gt->GetUtilities();
      std::copy(utils.begin(), utils.end(), std::back_inserter(dependencies));
    } else {
      cmTargetDependSet const& targetDeps = gg->GetTargetDirectDepends(gt);
  ...
  static void DetectTargetObjectDependencies(
    cmGlobalFastbuildGenerator* gg, cmGeneratorTarget* gt,
  ...
  struct DependencySorter
  {
    ...
    static void Sort(TTypeHelper& helper, std::vector<TType*>& entries)
    {
      ...
      typedef std::vector<TType*> OrderedEntrySet;
      typedef std::map<std::string, TType*> OutputMap;
      ...
      typedef std::map<TType*, std::vector<TType*> > DepMap;
      DepMap forwardDeps;
      DepMap reverseDeps;
      for (typename OrderedEntrySet::iterator iter = entries.begin();
           iter != entries.end(); ++iter) {
        TType* entry = *iter;
        std::vector<TType*>& entryInputs = forwardDeps[entry];
        ...
        for (StringVector::const_iterator inIter = inputs.begin();
          ...
          if (findResult != outputMap.end()) {
            TType* dentry = findResult->second;
      ...
      while (!forwardDeps.empty() && written) {
        written = false;
        for (auto iter = forwardDeps.begin(); iter != forwardDeps.end();
             ++iter) {
          std::vector<TType*>& fwdDeps = iter->second;
          TType* entry = iter->first;
          if (!fwdDeps.empty()) {
          ...
          std::vector<TType*>& revDeps = reverseDeps[entry];
          for (unsigned int i = 0; i < revDeps.size(); ++i) {
            TType* revDep = revDeps[i];

            // Fetch the list of deps on that target
            std::vector<TType*>& revDepFwdDeps = forwardDeps[revDep];
  ...
  typedef std::vector<cmGeneratorTarget*> OrderedTargetSet;
    ...
    cmGlobalFastbuildGenerator* gg, OrderedTargetSet& orderedTargets)
  {
    ...
    for (TargetDependSet::iterator iter = projectTargets.begin();
      ...
      orderedTargets.push_back(const_cast<cmGeneratorTarget*>(&target));
    }
  ...
  struct RemovalTest
  {
    bool operator()(const cmGeneratorTarget* gt) const
    {
      if (gt->GetType() == cmStateEnums::GLOBAL_TARGET) {
        // We only want to process global targets that live in the home
        // (i.e. top-level) directory.  CMake creates copies of these targets
        // in every directory, which we don't need.
        cmMakefile* mf = gt->Target->GetMakefile();
        if (strcmp(mf->GetCurrentSourceDirectory(), mf->GetHomeDirectory()) !=
            0) {
          return true;
        }
      }
...
public:
                                  ...
                                  TargetContextMap& map)
  {
    ...
    for (; it != end; ++it) {
      ...
      for (std::vector<cmLocalGenerator*>::const_iterator iter =
        ...
        for (auto targetIter = tgts.begin(); targetIter != tgts.end();
             ++targetIter) {
          cmGeneratorTarget* const gt = *targetIter;

          if (gg->IsRootOnlyTarget(gt) &&
              gt->Target->GetMakefile() != root->GetMakefile()) {
            continue;
          }

          TargetGenerationContext targetContext = { gt, root, generators, lg };
  ...
  static void WriteCustomBuildSteps(
    ...
    const cmGeneratorTarget* gt, const std::vector<cmCustomCommand>& commands,
                                    ...
                                    cmGeneratorTarget* gt)
  {
    ...
    std::vector<std::string> dependencies;
    Detection::DetectTargetCompileDependencies(context.self, gt, dependencies);
    ...
    for (; configIter != configEnd; ++configIter) {
      ...
      // Write out the output paths for the outcome of this target
      {
        ...
        Detection::DetectOutput(targetNames, gt, configName);
    ...
    WriteCustomBuildSteps(context, lg, gt, gt->GetPreBuildCommands(),
                          "PreBuild", dependencies);
    WriteCustomBuildSteps(context, lg, gt, gt->GetPreLinkCommands(), "PreLink",
                          dependencies);
    ...
    for (; configIter != configEnd; ++configIter) {
      ...
      // The prelink and prebuild commands
      {
        ...
        if (!gt->GetPreBuildCommands().empty()) {
          preBuildSteps.push_back("PreBuild");
        }
        if (!gt->GetPreLinkCommands().empty()) {
    ...
    for (std::set<std::string>::iterator langIter = languages.begin();
      ...
      for (; configIter != configEnd; ++configIter) {
        ...
        std::string baseCompileFlags;
        {
          ...
          std::string compileCmd;
          Detection::DetectBaseCompileCommand(compileCmd, lg, gt,
                                              objectGroupLanguage);
        ...
        context.fc.WriteComment("Source files:");
        {
          ...
          for (std::vector<cmSourceFile const*>::iterator sourceIter =
            ...
            Detection::DetectCompilerFlags(compilerFlags, lg, gt, srcFile,
                                           objectGroupLanguage, configName);
            std::string compileDefines = Detection::ComputeDefines(
              lg, gt, srcFile, configName, objectGroupLanguage);
        ...
        for (std::map<std::string, CompileCommand>::iterator groupIter =
          ...
          for (objectListIt = command.sourceFiles.begin();
            ...
            std::string targetCompileOutDirectory =
              Detection::DetectTargetCompileOutputDir(lg, gt, configName);
            context.fc.WriteVariable(
              "CompilerOutputPath",
    ...
    // Object libraries do not have linker stages
    // nor utilities
    bool hasLinkerStage = gt->GetType() != cmStateEnums::OBJECT_LIBRARY &&
      ...
      if (hasLinkerStage) {
        ...
        // Linker options
        {
          ...
          CM_AUTO_PTR<cmLinkLineComputer> linkLineComputer(
            context.self->CreateLinkLineComputer(context.self->GetLocalGenerators()[0],
              context.self->GetLocalGenerators()[0]->GetStateSnapshot().GetDirectory()));

          lg->GetTargetFlags(linkLineComputer.get(),configName, linkLibs, targetFlags, linkFlags,
                             frameworkPath, dummyLinkPath, gt);

          std::string linkPath;
          Detection::DetectLinkerLibPaths(linkPath, lg, gt, configName);
          ...
          // Remove the command from the front and leave the flags behind
          std::string linkCmd;
          if (!Detection::DetectBaseLinkerCommand(linkCmd, lg, gt,
                                                  configName)) {
            return;
          }
          ...
          std::string language = gt->GetLinkerLanguage(configName);
    ...
    WriteCustomBuildSteps(context, lg, gt, gt->GetPostBuildCommands(),
    ...
    if (!gt->GetPostBuildCommands().empty()) {
      orderDeps.push_back("PostBuild");
    }

    // Output a list of aliases
    WriteTargetAliases(context, gt, linkableDeps, orderDeps);

    context.fc.WritePopScope();
  }

  static void WriteTargetAliases(GenerationContext& context,
                                 cmGeneratorTarget* gt,
  ...
  static void WriteTargetUtilityDefinition(GenerationContext& context,
                                           ...
                                           cmGeneratorTarget* gt)
  {
    WriteTargetDefinition(context, lg, gt);
  }
                                     ...
                                     bool outputGlobals)
  {
    ...
    for (OrderedTargets::iterator targetIter = context.orderedTargets.begin();
      ...
      cmGeneratorTarget* constTarget = (*targetIter);
  ...
  static void WriteAliases(GenerationContext& context, bool outputGlobals)
  {
    ...
    for (OrderedTargets::iterator targetIter = context.orderedTargets.begin();
      ...
      cmGeneratorTarget* gt = findResult->second.target;
      const std::string& targetName = gt->GetName();
  ...
  cmGeneratorTarget* gt) const
{
  ...
  std::string dir;
  dir += target->GetMakefile()->GetCurrentBinaryDirectory();
  dir += "/";
