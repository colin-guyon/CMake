			...
			for (; configIter != configEnd; ++configIter)
			{
					...
					++groupIter)
				{
					...
					for(objectListIt = command.sourceFiles.begin(); objectListIt != command.sourceFiles.end(); ++objectListIt)
					{
						...
						// would both end in a aaa/item.cxx.obj file (see for ex. the Qt*Autogen.Parallel* tests)
						//
						// For information, to avoid conflicts when using Ninja, unique paths like this can be choosen:
						// (comes from the the Qt4Autogen.Parallel1 test)
						// CMakeFiles\parallel1.dir\E_\work\CMake_J_up_cleaning\Tests\QtAutogen\Parallel\bbb\aaa\item.cpp.obj
						// CMakeFiles\parallel1.dir\E_\work\CMake_J_up_cleaning\Tests\QtAutogen\Parallel\aaa\item.cpp.obj
						//
